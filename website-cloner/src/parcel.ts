export interface BlockedScrapeRecord {
  url: string;
  status: "blocked";
  reason: string;
 ...
 */
export async function saveBlockedAttempt(record: Omit<BlockedScrapeRecord, "timestamp" | "status">): Promise<void> {
  const fullRecord: BlockedScrapeRecord = {
    ...record,
    status: "blocked",
